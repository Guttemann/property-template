...
1. Resten av Fase 2 (punkt 4–5) — punkt 1 (sjekkliste), punkt 2 (`NUMBER_LOCALES`) og punkt 3 (unit-tester) er nå ferdig. De to gjenstående (`||`/`??`-inkonsekvens + dødt `locationBody2`-felt, JSON Schema) er fortsatt ikke startet. Sjekklist-arbeidet avdekket også at `FORMSPREE_ENDPOINT` (script.js) bør vurderes flyttet til config i en egen runde — se "Nye funn denne økten" over. Det ble bevisst **ikke** gjort som del av `NUMBER_LOCALES`- eller test-batchen, siden det har en annen alvorlighetsgrad (🔴 feilsendte bestillinger) og ikke ble bedt om sammen med noen av de to.
