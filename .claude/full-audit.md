...
- *(denne dokumentasjonscommiten, 2026-08-29)* — **Property-seed template-repo opprettet og verifisert.** `Guttemann/property-seed` (privat) opprettet fra det nøyaktige innholdet i `template/` (eget seed-repo, initial-commit `08dd250`, ingen `property-template`-historikk), GitHubs «Template repository»-innstilling på. `DEPLOYMENT.md` Steg 1 skrevet om («Use this template» som primærvei + repo-URL, manuell kopi beholdt som fallback) og Model-punktet justert så det ikke motsier Steg 1; `template/README.md` sin «ny property»-workflow oppdatert tilsvarende (den allerede godkjente Fase 1-teksten, tatt med her). Verifisert: «Use this template» → throwaway `property-seed-verify` → uavhengig repo (egen 1-commit-historikk `9e83cc2`, ikke seed-ens `08dd250`), identisk committed tre-SHA `a5e721a…`, `node scripts/validate-config.js` → VALID, `node --test` → 64/64. Kun dokumentasjon + nytt GitHub-repo, ingen runtime-/template-endring. Se "Property-seed template-repo — opprettet og verifisert (2026-08-29)" under.
...
### Opprydningsbatch etter Property #2-review (2026-08-21)

Liten, avgrenset batch bestilt direkte etter en post-Property #2-review — ikke en ny roadmap-fase. Nøyaktig tre punkter, ingen andre endringer: ingen refaktorering utover selve funnene, ingen nye features, ingen backend-/booking-arkitekturendring, `site-data.js` for Property #1 (repo-roten) **ikke rørt** (verifisert med `diff` før/etter). `property-2/site-data.js` heller ikke rørt — kun de delte runtime-/verktøyfilene som begge properties kjører på.

1. ✅ **`getText()`/`t()`-inkonsekvensen (§06) rettet.** Begge bruker nå samme "tom streng = ikke oversatt, fall tilbake"-semantikk. `getText()`s kropp er flyttet ut til en ny, DOM-fri `resolveLocalizedText(value, language, fallbackLanguage)` i [booking-logic.js](../booking-logic.js) — samme additive-parameter/DOM-frie mønster som allerede brukt for `formatMoney()` i Fase 2.3 — og `getText()` i script.js er nå en ren én-linjes delegering til den, uendret oppførsel for alle eksisterende kallsteder. `t()` kunne **ikke** flyttes ut på samme måte uten å restrukturere hvordan `translations.js` sin dict-av-dicts lastes (ville krysset inn i arkitektur, utenfor scope for denne batchen) — rettet i stedet direkte i script.js, samme `||`-semantikk, med en kommentar som peker til `resolveLocalizedText()` sin begrunnelse.

   **Verifisert før endring:** ingen felt i `translations.js` har i dag en legitim tom streng-verdi (grep gjennomgått), så endringen fra `??` til `||` i `t()` endrer ikke oppførsel for noe eksisterende innhold — den lukker kun det fremtidige hullet audit-en pekte på.

   **Propagert til begge properties:** siden dette er en delt-runtime-bugfiks (ikke Property #1-spesifikt innhold), ble identisk endring gjort i `property-2/script.js` og `property-2/booking-logic.js` — en reell, bevisst utført instans av akkurat den manuelle bugfiks-propageringen §01/§19 i original-audit-en beskriver som fremtidig teknisk gjeld. Begge filpar (`script.js`, `booking-logic.js`) bekreftet byte-identiske mellom rot og `property-2/` både før og etter endringen (`diff`).

   **Tester:** 7 nye tester i [tests/localized-text.test.js](../tests/localized-text.test.js) (mirrored uendret i `property-2/tests/localized-text.test.js`) for `resolveLocalizedText()`: streng-passthrough, normal per-språk-oppslag, manglende språknøkkel, og — selve regresjonen dette lukker — en eksplisitt tom streng som korrekt faller tilbake i stedet for å rendres tom. `t()` selv har ingen egen automatisert test (fortsatt DOM-/global-avhengig, ikke flyttet ut) — verifisert i stedet ved kodeinspeksjon (identisk én-linjes fallback-mønster som den nå testede `resolveLocalizedText()`).

2. ✅ **`node --test`-konvensjonen for flere property-mapper formalisert som en fast regel**, ikke bare et "funn" — se oppdatert [PROPERTY-CHECKLIST.md](../PROPERTY-CHECKLIST.md) seksjon 10. Ingen egen test-discovery-/ekskluderingslogikk bygget (eksplisitt utenfor scope) — fiksen er prosedural (kjør fra riktig mappe), ikke kode.

   **Ny nyanse funnet under dokumenteringen:** "`cd` inn i property-mappen"-regelen isolerer `property-2/` sitt eget tall helt rent (64 tester), men kan **ikke** isolere repo-rotens/Property #1 sitt eget tall — siden `property-2/` fysisk ligger *inni* rot-mappens filtre, plukker `node --test` kjørt fra roten alltid også opp `property-2/tests/` rekursivt (128 tester = 64 + 64, verifisert direkte denne økten). Dette er en strukturell konsekvens av at `property-2/` nå lever permanent som en undermappe i samme repo (ikke lenger en midlertidig monorepo-test), ikke en bug denne batchen forsøker å fikse. I en ekte produksjonssetting (hver property i sitt eget, separate repo) oppstår ikke denne asymmetrien.

3. ✅ **`scripts/validate-config.js` sjekker nå at bildefiler faktisk finnes på disk** for `heroImage`, `aboutImage.src` og hver `gallery[].src` — Node sin innebygde `fs`/`path`, ingen ny dependency. Kjøres kun **etter** at selve ajv-cli-schema-sjekken går grønn; mangler én eller flere filer, rapporteres nøyaktig felt + forventet sti, og scriptet avslutter med exit-kode `1` (samme kode som et vanlig schema-brudd), ikke `0`. URL-er (`http(s)://`, `//`) hoppes bevisst over i stedet for å sjekkes mot disk, i tilfelle config-formatet noensinne endrer seg til å tillate eksterne bilde-URLer (ingen slik bruk finnes i dag). **Fortsatt ikke sjekket:** at `gallery[].width`/`height` stemmer med filens faktiske pikseldimensjoner — det krever å dekode bildeheaderen, en egen, bevisst ikke-bygget sjekk. Identisk endring kopiert til `property-2/scripts/validate-config.js` (fungerer uendret der via sin egen `__dirname`-relative `ROOT`, samme mekanisme som allerede var etablert for dette scriptet).

**Testet, alt kjørt reelt (ikke antatt):**

- `node scripts/validate-config.js` (Property #1, repo-rot) → `VALID — data matches schema/property-config.schema.json, and all referenced image files exist on disk.`, exit 0.
- `node property-2/scripts/validate-config.js` (kjørt som `cd property-2 && node scripts/validate-config.js`) → samme `VALID`-melding, exit 0.
- `cd property-2 && node --test` → **64/64 grønne** (57 eksisterende + 7 nye `resolveLocalizedText()`-tester).
- `node --test` kjørt fra repo-roten → **128/128 grønne** (64 fra roten + 64 fra `property-2/`, se punkt 2 sin asymmetri-forklaring over — dette er forventet, ikke en feil).
- `diff` av `script.js`/`booking-logic.js`/`scripts/validate-config.js`/`tests/localized-text.test.js` mellom rot og `property-2/` → identiske i alle fire par, både før og etter denne batchen.
- Ingen endring i `site-data.js` for noen av de to propertyene (kun de delte runtime-/verktøyfilene rørt).
...
- Ingen `property-seed`-template-repo på GitHub ennå — «Use this template» som gjør den rene kopieringen er planlagt, men til da kopieres `template/`-innholdet for hånd (slik sandbox-testen gjorde). *(Oppdatering 2026-08-29: `property-seed`-repoet er nå opprettet og verifisert — se "Property-seed template-repo — opprettet og verifisert (2026-08-29)" under. Dette punktet står som historisk kontekst for hva Deployment Batch 1 bevisst ikke omfattet.)*
...
### Property-seed template-repo — opprettet og verifisert (2026-08-29)

Liten dokumentasjonsbatch som lukker det siste punktet fra «Bevisst ikke med i denne batchen» over: den manuelle «kopier `template/`-innholdet for hånd»-steget er nå erstattet av et ekte GitHub template-repo.

**Opprettet:**

- ✅ **`Guttemann/property-seed`** (privat GitHub-repo) — seedet fra det nøyaktige innholdet i `property-template/template/` lagt i repo-roten (ingen `template/`-undermappe), eget fresh initial-commit `08dd250`, ingen git-historikk fra `property-template`. GitHubs **«Template repository»-innstilling er på**, så «Use this template» gir en ren ett-commit-kopi.
- ✅ **[DEPLOYMENT.md](../DEPLOYMENT.md) Steg 1** skrevet om: «Use this template» fra `https://github.com/Guttemann/property-seed` er nå **primærvei** (privat repo, navn `property-<slug>`, én fresh commit, ingen delt historikk), med den manuelle `template/`-kopien beholdt som eksplisitt merket fallback. «Model»-punktet øverst i filen er justert tilsvarende så det ikke motsier Steg 1.
- ✅ **[template/README.md](../template/README.md)** — «Creating a new property» sitt steg 1 gjør «Use this template» til primærvei og beholder den manuelle bootstrappen som merket fallback; intro-avsnittet og `PROPERTY-CHECKLIST.md`-referansen er omskrevet så README-en gir mening lest inne i et frittstående property-repo, ikke bare i denne monorepoen. (Dette er den allerede godkjente Fase 1-teksten, tatt med i denne commiten.)

**Verifisert med et ekte «Use this template»-kjør (2026-08-29):**

Et separat, disposabelt test-repo — *ikke* `property-template`, `property-seed` eller noen reell property:

- `Guttemann/property-seed-verify` (privat) opprettet via GitHubs «Use this template» og klonet lokalt til en scratch-mappe utenfor alle ekte repoer.
- **Uavhengig:** eneste remote er `property-seed-verify`; historikken er nøyaktig **1 commit** (`9e83cc2`, GitHubs egen «Initial commit») — seed-repoets `08dd250` finnes **ikke** i historikken. Ren `git status`.
- **Innholdsidentisk:** committed tre-SHA er `a5e721a92d4e8f7f82e9909798dc6b7424812c93` i både `property-seed-verify@9e83cc2` og `property-seed@08dd250` — bit-for-bit samme fil-tre (29 filer).
- `node scripts/validate-config.js` → **VALID** (exit 0). `node --test` → **64/64 grønne**.
- Lokal klon slettet etter verifisering; `property-template`, `property-seed`, `property-2/` og alle reelle properties urørt gjennom hele testen.

**CRLF-notat (ikke en blokker, bevisst ikke adressert):** `git clone` på denne Windows-maskinen har `core.autocrlf=true`, så arbeidskopien fra klonen fikk CRLF-linjeslutt mens den lokale `property-seed`-mappen (bygget med `cp` fra LF-kilder, aldri re-checket-ut) er LF. Bekreftet at dette *kun* er et checkout-artefakt: git sin egen tre-SHA og en CR-strippet `diff -r` er begge identiske. Ufarlig for Node og Netlify. Ingen `.gitattributes` lagt til — hvis deterministisk LF på tvers av plattformer noen gang blir ønsket er det en egen, senere vurdering.

**Forholdet til `property-sandbox` (Deployment Batch 1):** `property-sandbox` var en engangs manuell håndkopi for å bevise GitHub→Netlify-deploy. `property-seed` erstatter nå den manuelle kopieringsmekanismen for alle fremtidige properties; `property-sandbox` kan arkiveres/slettes når som helst uten konsekvens.

Ingen runtime-kode, schema, tester, `template/`-funksjonalitet eller deploy-oppførsel er endret i denne batchen — kun `DEPLOYMENT.md`, `template/README.md`, denne filen, og et nytt separat GitHub-repo.
...
**Deployment:** Deployment Batch 1 (2026-08-27) la til `DEPLOYMENT.md` + `template/netlify.toml` (og deploy-avsnittene i `template/README.md` / `PROPERTY-CHECKLIST.md`) og ble verifisert med en ekte, disposabel Netlify sandbox-deploy — eget `property-sandbox`-repo, *ikke* `property-template` eller noen kunde-property; commit `81f5c96`; live over HTTPS; `/robots.txt` og `/sitemap.xml` oppførte seg som forventet. Kun dokumentasjon/konfig — ingen runtime- eller template-endring. Se "Deployment Batch 1 — Netlify-runbook + sandbox-deploy verifisert" under. **Property-seed template-repo (2026-08-29):** `Guttemann/property-seed` (privat) er nå opprettet fra `template/`-innholdet med GitHubs «Template repository»-innstilling på, og «Use this template» er verifisert ende-til-ende mot et throwaway `property-seed-verify` (uavhengig repo, 1-commit-historikk, identisk tre-SHA, `validate-config.js` VALID, `node --test` 64/64). `DEPLOYMENT.md` Steg 1 og `template/README.md` er oppdatert til å bruke «Use this template» som primærvei. Se "Property-seed template-repo — opprettet og verifisert (2026-08-29)" under.
...
1. ✅ **Ferdig** — (a) opprydningsbatchen 2026-08-21 rettet den siste resten av Fase 2, `||`/`??`-inkonsekvensen i `getText()`/`t()` (§06); Fase 2 sin opprinnelige 5-punktsliste er dermed **fullstendig ferdig**. (b) **Deployment Batch 1** (2026-08-27) la til Netlify deploy-runbooken og ble verifisert med en ekte throwaway sandbox-deploy. (c) **Property-seed template-repo** (2026-08-29) opprettet og verifisert med «Use this template» — den manuelle «kopier `template/` for hånd»-steget er nå borte. Se "Opprydningsbatch etter Property #2-review", "Deployment Batch 1 — Netlify-runbook + sandbox-deploy verifisert" og "Property-seed template-repo — opprettet og verifisert (2026-08-29)" over.
2. **Første reelle go-live** (neste deployment-milepæl): følg [DEPLOYMENT.md](../DEPLOYMENT.md) Steg 1–6 ende-til-ende for en faktisk property (Steg 1 = «Use this template» fra `property-seed`). Når `seo.siteUrl` settes til et ekte domene: kjør `node scripts/sync-seo.js` på nytt (aktiverer canonical + fyller `sitemap.xml`), legg til `Sitemap:`-linjen i `robots.txt`, og vurder JSON-LD og hreflang/separate URL-er for thai på nytt. De eksakte Netlify-dashboard-stegene for domene/DNS/HTTPS pinnes i runbooken ved denne anledningen.
