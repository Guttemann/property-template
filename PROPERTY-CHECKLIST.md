...
- [ ] Kjør `node --test` **før** deploy — 57 tester dekker dato-/blokkering-/prislogikken (`booking-logic.js`) og JSON-schemaet (`schema/property-config.schema.json`), og feiler raskt hvis en fremtidig endring bryter noe her. Ingen `npm install` nødvendig for kjernetestene (Node sin innebygde testrunner) — schema-testene i `tests/schema-validation.test.js` bruker samme `npx ajv-cli@5.0.0` som over.
  **⚠️ Funn fra Property #2-byggeøkten (2026-08-21, Node v24.19.0):** `node --test <mappe>/` med en eksplisitt katalogsti (trailing slash) feiler med `MODULE_NOT_FOUND` — bruk bar `node --test` (auto-discovery) i stedet. Viktigere: `node --test` uten argumenter **oppdager rekursivt alle `tests/`-mapper under `cwd`**. Ligger en klonet property (f.eks. en `property-2/`-mappe) inne i *samme* repo som denne templaten — for eksempel midlertidig, under en template-test — vil `node --test` kjørt fra repo-roten telle **begge** propertyenes tester sammen (114 i stedet for 57) uten å si fra at det er to forskjellige sett. Kjør `cd <property-mappe> && node --test` for et property-isolert tall. I den vanlige produksjonssettingen (hver property i sitt eget repo) er dette irrelevant.
